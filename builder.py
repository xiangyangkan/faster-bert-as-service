...
# 第一个维度是否为BATCH_SIZE, Triton要求BATCH_SIZE维度为第0维
...
TRT_LOGGER = trt.Logger(trt.Logger.VERBOSE)  # 设置更详细的日志输出
...
handle = ctypes.CDLL(DLL_FILE[GPU_MODEL], mode=ctypes.RTLD_GLOBAL)
...
class BertConfig:
    def __init__(self, bert_config_path, use_fp16, use_int8, use_strict, use_fc2_gemm, use_int8_skipln,
                 use_int8_multihead, use_qat, use_sparsity, timing_cache):
...
def emb_layernorm(builder, network, config, weights_dict, builder_config, sequence_lengths, batch_sizes):
    ...
    input_shape = (-1 if len(sequence_lengths) > 1 else sequence_lengths[0],
    ...
    input_mask = network.add_input(name="input_mask", dtype=trt.int32, shape=input_shape)
...
def build_engine(batch_sizes, workspace_size, sequence_lengths, config, weights_dict, squad_json, vocab_file,
    ...
    with trt.Builder(TRT_LOGGER) as builder, builder.create_network(
        ...
        if ENABLE_DEBUG:
            last_transformer_transpose_layer = network.add_shuffle(bert_out)
            last_transformer_transpose_layer.second_transpose = (1, 0, 2, 3, 4)
            last_transformer_transpose_output = last_transformer_transpose_layer.get_output(0)
...
def generate_calibration_cache(sequence_lengths, workspace_size, config, weights_dict, squad_json, vocab_file,
                               calibrationCacheFile, calib_num):
    """
...
def main():
    ...
    parser.add_argument("-b", "--batch-size", default=[], action="append",
                        ...
                        type=int)
    parser.add_argument("-s", "--sequence-length", default=[], action="append",
                        help="Sequence length of the BERT model", type=int)
    ...
    parser.add_argument("-w", "--workspace-size", default=1000,
    ...
    parser.add_argument("-tcf", "--timing-cache-file",
                        help="Path to tensorrt build timeing cache file, only available for tensorrt 8.0 and later",
                        required=False)
    ...
    config = BertConfig(bert_config_path, args.fp16, args.int8, args.strict, args.force_fc2_gemm,
                        args.force_int8_skipln, args.force_int8_multihead, args.int8 and args.onnx != None, args.sparse,
                        args.timing_cache_file)

    if args.calib_path != None:
        calib_cache = args.calib_path
    else:
        calib_cache = "BertSquadL{}H{}A{}S{}CalibCache".format(config.num_hidden_layers, config.head_size,
                                                               config.num_attention_heads,
                                                               "-".join(str(len) for len in args.sequence_length))
    ...
    if args.onnx != None:
        ...
        generate_calibration_cache(args.sequence_length, args.workspace_size, config, weights_dict, args.squad_json,
                                   args.vocab_file, calib_cache, args.calib_num)
    else:
        raise RuntimeError(
            "You need either specify TF checkpoint using option --ckpt or ONNX using option --onnx to build TRT BERT model.")

    with build_engine(args.batch_size, args.workspace_size, args.sequence_length, config, weights_dict, args.squad_json,
